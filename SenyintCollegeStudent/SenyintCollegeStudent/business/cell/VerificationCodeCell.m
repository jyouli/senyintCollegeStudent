...
- (void)setVerificationCodeBtn
{
    ...
    [btn setAttributedTitle:[NSString getAttributedStringFromString:@"获取验证码" Color:BodyContentImportantText_Font_Color Fount:TextFieldInputText_Font_Size] forState:UIControlStateNormal];
...
- (void)setModel:(InfoTextFieldCellModel *)model
{
    ...
    if (countdown == Countdown_Second) {
        [countdownBtn setAttributedTitle:[NSString getAttributedStringFromString:@"获取验证码" Color:[UIColor redColor] Fount:TextFieldInputText_Font_Size] forState:UIControlStateNormal];
    ...
    if ([Countdown_UserRegist isEqualToString:model.verificationCodeCountdownKey]) {
        [VerificationCodeCountdownSingle sharedCodeCountdownSingle].userRegistUpdateUI = ^(NSInteger countdown){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (countdown == 0) {
                    [safeBtn setAttributedTitle:[NSString getAttributedStringFromString:@"获取验证码" Color:[UIColor redColor] Fount:TextFieldInputText_Font_Size] forState:UIControlStateNormal];
                    safeBtn.enabled = YES;
                } else {
                    [safeBtn setAttributedTitle:[NSString getAttributedStringFromString:[NSString stringWithFormat:@"%lds后重发",countdown] Color:Disabledgray_Color Fount:TextFieldInputText_Font_Size] forState:UIControlStateDisabled];
    ...
    if ([Countdown_ForgetPassWord isEqualToString:model.verificationCodeCountdownKey]) {
        ...
        [VerificationCodeCountdownSingle sharedCodeCountdownSingle].forgetPassWordUpdateUI = ^(NSInteger countdown){
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (countdown == 0) {
                    safeBtn.layer.borderColor = [BodyContentImportantText_Font_Color CGColor];
                    [safeBtn setAttributedTitle:[NSString getAttributedStringFromString:@"获取验证码" Color:[UIColor redColor] Fount:TextFieldInputText_Font_Size] forState:UIControlStateNormal];
                    safeBtn.enabled = YES;
                } else {
                    [safeBtn setAttributedTitle:[NSString getAttributedStringFromString:[NSString stringWithFormat:@"%lds后重发",countdown] Color:Disabledgray_Color Fount:TextFieldInputText_Font_Size] forState:UIControlStateDisabled];
